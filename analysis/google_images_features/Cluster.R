...
pathdata <- here::here("data","BIG_FILES","images")

pathresults <- here::here("results")
pathphoto_png <- here::here(pathdata,"png_sq")
...
cluster <- function (n_im,clust)
{
  ...
  rownames(ps.data) <- ps.data$patchID
  cl.data = SDMTools::ClassStat(patch_mat)
  rownames(cl.data) <- cl.data$class
  
  patch_data <- merge(ps.data,cl.data,by = 'row.names')
  
  patch_data <- patch_data[,c("patchID","n.cell","n.core.cell","n.edges.perimeter",
                           "area","core.area","perimeter","perim.area.ratio",
                           "shape.index","frac.dim.index","core.area.index",
                           "perimeter.area.frac.dim","mean.frac.dim.index",
                           "aggregation.index","effective.mesh.size")]

  rm(kvalues,patch,df)
  
  #output
  
  cbind.data.frame(name,as.data.frame(kmeans$centers),patch_data)
...
cluster.lab.plot <- function (n_im,clust,lg_sz,ncol,FISH)
{
  ...
  df_rgb <-  na.omit(as.data.frame(im_no_NA,wide="c")) %>% dplyr::mutate(rgb.val=rgb(c.1,c.2,c.3))
...
cluster_stat <- function(cluster_raw,n_im,clust)
{
  #n_im=114#
  ...
  shape_id <- c("SH_n.cell_mean","SH_n.cell_sd","SH_n.core.cell_mean","SH_n.core.cell_sd",
                "SH_perimeter_mean","SH_perimeter_sd","SH_perim.area.ratio_mean",
                "SH_perim.area.ratio_sd","SH_core.area.index_mean", "SH_core.area.index_sd",
                "SH_shape.index_mean","SH_shape.index_sd","SH_frac.dim.index_mean",
                "SH_mean.frac.dim.index","SH_aggregation.index","SH_effective.mesh.size")
  ...
  SH_n.cell_mean <- mean(dat[,"n.cell"])
  SH_n.cell_sd<- sd(dat[,"n.cell"])
  ...
  SH_shape.index_mean <- mean(dat[,"shape.index"])
  SH_shape.index_sd <- sd(dat[,"shape.index"])
  SH_frac.dim.index_mean <- mean(dat[,"frac.dim.index"])
  SH_mean.frac.dim.index <- mean(dat[,"mean.frac.dim.index"])
  SH_aggregation.index <- mean(dat[,"aggregation.index"])
  SH_effective.mesh.size <- mean(dat[,"effective.mesh.size"])
...
#EXEMPLE WITH THE BASIC FUNCTIONS FIGURE S1 A (with who <- "flo")----

  exsp <- "006_1_52758154102_o"
  n_im=which(grepl(paste0(exsp,".png"),files))
  ...
  ##FIGURE S1 B
    ...
    ggplot2::ggsave(filename = here::here("figures_tables","images_features","coor_clusters.jpg"),
                    plot = plot, 
...
#WARNING Do not run as all images are not available; the output files in 
#results/images_features have already been computed with all images

  pict <-length(files)
  
  raw_data <- as.data.frame(do.call(rbind,pbmcapply::pbmclapply(1:pict, function(i) cluster(n_im=i,clust=9),mc.cores = parallel::detectCores() )))
  stat_data <- as.data.frame(do.call(rbind,pbmcapply::pbmclapply(1:pict, function(i) cluster_stat(cluster_raw=raw_data,n_im=i,clust=9),mc.cores = parallel::detectCores() )))
  write.csv(stat_data,here::here("results","images_features","cluster.csv"))
